...
export function ClientSystemDetailDialog({ record, open, onClose }: { record: ClientMonitoringRecord; open: boolean; onClose: () => void }) {
  ...
  return <DDialog open={open} onClose={onClose} size="xl" title={record.installation.name} description={`${record.clientProduct.productName} · ${record.installation.environment}`} footer={<div className="client-dialog-actions"><DButton variant="outline" onClick={onClose}>Close</DButton></div>}>
    <DTabs defaultValue="overview" value={tab} onValueChange={setTab} className="client-detail-tabs">
      ...
      <DTabsContent value="overview"><dl className="client-monitoring-detail-list"><div><dt>Product</dt><dd>{record.clientProduct.productName}</dd></div><div><dt>Installation</dt><dd>{record.installation.name}</dd></div><div><dt>Environment</dt><dd>{record.installation.environment}</dd></div><div><dt>Deployment mode</dt><dd>{record.installation.deploymentMode}</dd></div><div><dt>Health</dt><dd><HealthBadge status={record.health} /></dd></div><div><dt>Maintenance</dt><dd>{record.maintenanceState === 'NOT_APPLICABLE' ? 'Not applicable for shared deployment' : record.maintenanceState}</dd></div><div><dt>Current version</dt><dd>{record.currentDeployment?.version ?? 'Not deployed'}</dd></div><div><dt>Last observed</dt><dd>{formatDate(record.lastObservedAt)}</dd></div><div><dt>Runtime status</dt><dd><ObservedStateBadge state={record.runtimeStatus} /></dd></div><div><dt>Active incidents</dt><dd>{record.activeIncidents.length}</dd></div>{dedicated ? <div><dt>Bound infrastructure</dt><dd>{record.infrastructureBindings.length}</dd></div> : null}</dl></DTabsContent>
